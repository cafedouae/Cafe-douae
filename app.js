...
/* ================================================================
   NAVIGATION
================================================================ */
const VIEW_TITLES={caisse:"Caisse",tickets:"Tickets",stock:"Stock",produits:"Menu & Produits",serveurs:"Serveurs",dashboard:"Statistiques",rapport:"Rapport des ventes"};
...
/* ================================================================
   RAPPORT DES VENTES
================================================================ */
let rapportSection = "all";

function setRapportSection(sec, btn){
  rapportSection = sec;
  document.querySelectorAll("[data-rpsec]").forEach(b=>b.classList.toggle("active", b.dataset.rpsec===sec));
  renderRapportContent(window._rapportData || []);
}
...
async function loadRapport(){
  ...
  if(!from||!to){ toast("Sélectionne une période",true); return; }
  ...
  if(!tks||!tks.length){
    document.getElementById("rapport-content").innerHTML=`<div class="empty">Aucune vente sur cette période.</div>`;
    ["rp-total-cafe","rp-total-terrasse","rp-total-all","rp-nb-tickets"].forEach(id=>document.getElementById(id).textContent="0 DH");
    document.getElementById("rp-nb-tickets").textContent="0";
    return;
  }
  ...
  // Attach items and section to each ticket
  const data = tks.map(t=>{
    const items = (allItems||[]).filter(i=>i.ticket_id===t.id);
    // Determine section: if ALL items are terrasse → terrasse, if ALL café → café, else mixed
    const sections = items.map(i=>{
      const prod = products.find(p=>p.id===i.product_id);
      return prod ? catSection(prod.category_id) : "cafe";
    });
    const sec = sections.every(s=>s==="terrasse") ? "terrasse" : sections.every(s=>s==="cafe") ? "cafe" : "mixte";
    return { ...t, items, sec };
  });

  window._rapportData = data;

  // KPIs
  const cafeRev   = data.filter(t=>t.sec!=="terrasse").reduce((s,t)=>s+Number(t.total),0);
  const terRev    = data.filter(t=>t.sec!=="cafe").reduce((s,t)=>s+Number(t.total),0);
  // For mixed tickets, split by item section
  let cafeTotal=0, terTotal=0;
  data.forEach(t=>{
    t.items.forEach(i=>{
      const prod=products.find(p=>p.id===i.product_id);
      const s=prod?catSection(prod.category_id):"cafe";
      if(s==="terrasse") terTotal+=Number(i.subtotal);
      else cafeTotal+=Number(i.subtotal);
    });
  });
  document.getElementById("rp-total-cafe").textContent     = dh(cafeTotal);
  document.getElementById("rp-total-terrasse").textContent = dh(terTotal);
  document.getElementById("rp-total-all").textContent      = dh(cafeTotal+terTotal);
  document.getElementById("rp-nb-tickets").textContent     = data.length+" ticket"+(data.length>1?"s":"");

  renderRapportContent(data);
}

function renderRapportContent(data){
  const wrap = document.getElementById("rapport-content");
  if(!data.length){ wrap.innerHTML=`<div class="empty">Aucune donnée.</div>`; return; }

  // Filter by section
  let filtered = data;
  if(rapportSection==="cafe")     filtered = data.filter(t=> t.items.some(i=>{ const p=products.find(x=>x.id===i.product_id); return p&&catSection(p.category_id)==="cafe"; }));
  if(rapportSection==="terrasse") filtered = data.filter(t=> t.items.some(i=>{ const p=products.find(x=>x.id===i.product_id); return p&&catSection(p.category_id)==="terrasse"; }));

  // Group by day
  const byDay = {};
  filtered.forEach(t=>{
    const day = todayStr(new Date(t.created_at));
    if(!byDay[day]) byDay[day] = [];
    byDay[day].push(t);
  });

  wrap.innerHTML = Object.entries(byDay).sort((a,b)=>b[0].localeCompare(a[0])).map(([day, tickets])=>{
    const dayDate = new Date(day+"T12:00:00");
    const dayLabel = dayDate.toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long",year:"numeric"});
    const dayTotal = tickets.reduce((s,t)=>s+Number(t.total),0);

    // Split day items by section
    let cafeDayTotal=0, terDayTotal=0;
    tickets.forEach(t=> t.items.forEach(i=>{
      const prod=products.find(p=>p.id===i.product_id);
      const s=prod?catSection(prod.category_id):"cafe";
      if(s==="terrasse") terDayTotal+=Number(i.subtotal);
      else cafeDayTotal+=Number(i.subtotal);
    }));
      ...
      <div style="background:var(--bg);border-radius:10px;padding:12px 14px;margin-bottom:8px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          <div style="display:flex;align-items:center;gap:10px;">
            <span class="mono" style="font-size:12px;font-weight:700;">${t.ticket_number}</span>
            <span style="font-size:12px;color:var(--text2);">${fmtTime(t.created_at)} · 👤 ${t.employees?.name||"—"}</span>
          </div>
          <span class="mono" style="font-weight:700;color:var(--copper);">${dh(t.total)}</span>
        </div>
        <table style="width:100%;font-size:12.5px;">
          <thead><tr>
            <th style="padding:4px 8px;text-align:left;color:var(--text3);font-size:11px;">Produit</th>
            <th style="padding:4px 8px;text-align:center;color:var(--text3);font-size:11px;">Qté</th>
            <th style="padding:4px 8px;text-align:right;color:var(--text3);font-size:11px;">P.U.</th>
            <th style="padding:4px 8px;text-align:right;color:var(--text3);font-size:11px;">Total</th>
            <th style="padding:4px 8px;text-align:center;color:var(--text3);font-size:11px;">Section</th>
          </tr></thead>
          <tbody>
            ${t.items.map(i=>{
              const prod=products.find(p=>p.id===i.product_id);
              const sec=prod?catSection(prod.category_id):"cafe";
              const secLabel=sec==="terrasse"?"🏖️":"☕";
              const filtered_show = rapportSection==="all" ||
                (rapportSection==="cafe" && sec==="cafe") ||
                (rapportSection==="terrasse" && sec==="terrasse");
              return filtered_show?`<tr style="border-bottom:1px solid var(--border);">
                <td style="padding:5px 8px;">${i.product_name}</td>
                <td style="padding:5px 8px;text-align:center;font-weight:700;">${i.qty}</td>
                <td style="padding:5px 8px;text-align:right;" class="mono">${dh(i.unit_price)}</td>
                <td style="padding:5px 8px;text-align:right;font-weight:700;color:var(--copper);" class="mono">${dh(i.subtotal)}</td>
                <td style="padding:5px 8px;text-align:center;">${secLabel}</td>
              </tr>`:'';
            }).join("")}
          </tbody>
        </table>
        ${t.discount>0?`<div style="font-size:12px;color:var(--text2);margin-top:6px;text-align:right;">Remise : −${dh(t.discount)}</div>`:''}
      </div>`).join("");

    return `
      <div style="margin-bottom:20px;">
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:2px solid var(--copper);margin-bottom:12px;">
          <div>
            <div style="font-weight:700;font-size:15px;text-transform:capitalize;">${dayLabel}</div>
            <div style="font-size:12px;color:var(--text2);margin-top:2px;">
              ☕ ${dh(cafeDayTotal)} &nbsp;·&nbsp; 🏖️ ${dh(terDayTotal)} &nbsp;·&nbsp; ${tickets.length} ticket${tickets.length>1?"s":""}
            </div>
          </div>
          <div style="font-family:'Playfair Display',serif;font-size:20px;font-weight:700;color:var(--copper);">${dh(dayTotal)}</div>
        </div>
        ${ticketRows}
      </div>`;
  }).join("");
