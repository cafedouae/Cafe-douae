...
async function loadServersForPicker(){
  ...
  try{
    ...
    if(typeof _preloadedServers !== "undefined" && _preloadedServers){
      ...
      sb.from("employees").select("*").eq("active",true).in("role",["serveur","admin","gerant"]).order("name").then(r=>{if(typeof _preloadedServers!=="undefined")_preloadedServers=r.data||[];});
    } else {
      // Fetch all active employees who can serve (not just role=serveur)
      const res=await sb.from("employees").select("*").eq("active",true).in("role",["serveur","admin","gerant"]).order("name");
      data = res.data || [];
    ...
    try{
      const res2=await sb.from("employees").select("*").eq("active",true).order("name");
      data = res2.data || [];
    }catch(e2){ data=[]; }
  }
  grid.innerHTML="";
...
async function loadProducts(){
  const {data}=await sb.from("products").select("*,categories(sort_order)").eq("active",true).order("sort_order",{ascending:true}).order("name");
  products=data||[];
  renderProductGrid();
}
