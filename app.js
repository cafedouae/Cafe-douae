...
async function loadRapport(){
  ...
  if(!tks||!tks.length){
    ...
    document.getElementById("rp-nb-tickets-label").textContent="Aucun ticket";
    document.getElementById("rp-srv-tbody").innerHTML=`<tr><td colspan="5" class="empty">Aucune vente ce jour.</td></tr>`;
  ...
  document.getElementById("rp-nb-tickets-label").textContent=
    `${data.length} ticket${data.length>1?"s":""} · ☕ ${dh(cafeTotal)} · 🏖️ ${dh(terTotal)}`;
  ...
  data.forEach(t=>{
    ...
    t.items.forEach(i=>{
      ...
      if(sec==="terrasse"){
        srvMap[srv].ter+=v;
        if(mat) srvMap[srv].terMatin+=v;
        if(soir) srvMap[srv].terSoir+=v;
      } else {
        srvMap[srv].cafe+=v;
        if(mat) srvMap[srv].cafeMatin+=v;
        if(soir) srvMap[srv].cafeSoir+=v;
      }
    });
  });
  const tbody=document.getElementById("rp-srv-tbody");
  tbody.innerHTML=Object.entries(srvMap).sort((a,b)=>(b[1].cafe+b[1].ter)-(a[1].cafe+a[1].ter)).map(([name,v])=>`
    <tr><td colspan="5" style="padding:0;">
      <div style="padding:12px 14px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
          <div style="font-weight:700;font-size:15px;">👤 ${name}</div>
          <div style="font-weight:700;font-size:16px;" class="mono">${dh(v.cafe+v.ter)}</div>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:13px;">
          <div style="background:var(--bg);border-radius:8px;padding:10px 12px;border-left:3px solid var(--copper);">
            <div style="font-weight:700;color:var(--copper);margin-bottom:6px;">☕ Café</div>
            <div style="display:flex;justify-content:space-between;padding:3px 0;"><span style="color:var(--text2);">🌅 Matin</span><span class="mono">${dh(v.cafeMatin)}</span></div>
            <div style="display:flex;justify-content:space-between;padding:3px 0;"><span style="color:var(--text2);">🌆 Soir</span><span class="mono">${dh(v.cafeSoir)}</span></div>
            <div style="display:flex;justify-content:space-between;padding:5px 0;border-top:1px solid var(--border);margin-top:4px;font-weight:700;"><span>Total</span><span class="mono" style="color:var(--copper);">${dh(v.cafe)}</span></div>
          </div>
          <div style="background:var(--bg);border-radius:8px;padding:10px 12px;border-left:3px solid var(--sage);">
            <div style="font-weight:700;color:var(--sage);margin-bottom:6px;">🏖️ Terrasse</div>
            <div style="display:flex;justify-content:space-between;padding:3px 0;"><span style="color:var(--text2);">🌅 Matin</span><span class="mono">${dh(v.terMatin)}</span></div>
            <div style="display:flex;justify-content:space-between;padding:3px 0;"><span style="color:var(--text2);">🌆 Soir</span><span class="mono">${dh(v.terSoir)}</span></div>
            <div style="display:flex;justify-content:space-between;padding:5px 0;border-top:1px solid var(--border);margin-top:4px;font-weight:700;"><span>Total</span><span class="mono" style="color:var(--sage);">${dh(v.ter)}</span></div>
          </div>
        </div>
        <div style="text-align:right;font-size:11px;color:var(--text3);margin-top:6px;">${v.tickets} ticket${v.tickets>1?"s":""} · Moy. ${dh((v.cafe+v.ter)/v.tickets)}/ticket</div>
      </div>
    </td></tr>`).join("");
...
function renderRapportDetail(data){
  ...
  const sections=[
    {key:"cafe", label:"☕ Café", color:"var(--copper)"},
    {key:"terrasse", label:"🏖️ Terrasse", color:"var(--sage)"}
  ];
  wrap.innerHTML=sections.map(sec=>{
    ...
    const shiftBlocks=shifts.map(shift=>{
      ...
      if(!tickets.length) return `<div style="color:var(--text3);font-size:13px;padding:10px 0;">${shift.label} — Aucune vente</div>`;
      ...
      const rows=tickets.map(t=>{
        ...
        return `<div style="background:var(--bg);border-radius:8px;padding:10px 12px;margin-bottom:6px;">
          ...
          ${secItems.map(i=>`
            <div style="display:flex;justify-content:space-between;font-size:12.5px;padding:3px 0;border-top:1px dashed var(--border);">
      ...
      return `<div style="margin-bottom:14px;">
        <div style="display:flex;justify-content:space-between;font-size:13px;font-weight:600;padding:6px 0;border-bottom:1px solid var(--border);margin-bottom:8px;">
          <span style="color:var(--text2);">${shift.label}</span><span class="mono">${dh(shiftTotal)}</span>
        </div>${rows}
      </div>`;
    }).join("");
    const secTotal=data.reduce((s,t)=>s+t.items.filter(i=>itemSection(i)===sec.key).reduce((x,i)=>x+Number(i.subtotal),0),0);
    return `<div class="card" style="border-top:3px solid ${sec.color};margin-bottom:0;">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        <h3 style="margin:0;font-size:16px;color:${sec.color};">${sec.label}</h3>
        <span style="font-family:'Playfair Display',serif;font-size:22px;font-weight:700;color:${sec.color};">${dh(secTotal)}</span>
      </div>${shiftBlocks}
    </div>`;
  }).join("");
}
