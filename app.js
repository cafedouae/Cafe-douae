...
function getShift(iso){ const h=new Date(iso).getHours(); return (h>=14||h<3)?"🌆 Soir":"🌅 Matin"; }
...
async function loadTickets(){
  ...
  const soir=paid.filter(t=>{ const h=new Date(t.created_at).getHours(); return h>=14||h<3; });
...
async function loadDashboard(){
  ...
  const soir=all.filter(t=>{ const h=new Date(t.created_at).getHours(); return h>=14||h<3; });
  ...
  // By shift
  const byShift={"🌅 Matin (8h–14h)":{rev:0,cnt:0},"🌆 Soir (14h–03h)":{rev:0,cnt:0}};
  all.forEach(t=>{ const h=new Date(t.created_at).getHours(); const key=h>=8&&h<14?"🌅 Matin (8h–14h)":(h>=14||h<3)?"🌆 Soir (14h–03h)":null; if(key){ byShift[key].rev+=Number(t.total); byShift[key].cnt++; } });
...
function isSoir(iso){ const h=new Date(iso).getHours(); return h>=14||h<3; }
...
async function loadRapportSection(sec){
  ...
  const to=todayStr(nextDay)+"T03:00:00";
  ...
  const shifts=[
    ...
    {label:"🌆 Soir (14h–03h)", filter:t=>isSoir(t.created_at)}
  ];
