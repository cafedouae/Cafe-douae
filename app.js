...
function showHome(){
  ...
  hs.classList.remove("hidden"); hs.style.display="";
  const ss=document.getElementById("server-screen");
  ss.classList.remove("active"); ss.style.display="none";
  const als=document.getElementById("admin-login-screen");
  als.classList.remove("active"); als.style.display="none";
  const ap=document.getElementById("app");
  ap.classList.remove("active"); ap.style.display="";
}
function hideHome(){
  const hs=document.getElementById("home-screen");
  hs.classList.add("hidden");
  hs.style.display="none"; // compat old HTML
}
...
document.getElementById("btn-new-order").onclick=async()=>{
  ...
  hideHome();
  const ss=document.getElementById("server-screen");
  ss.classList.add("active"); ss.style.display="flex";
  // Load servers in background (non-blocking)
  loadServersForPicker();
};
document.getElementById("server-back").onclick=()=>{
  const ss2=document.getElementById("server-screen");
  ss2.classList.remove("active"); ss2.style.display="none";
  const hs2=document.getElementById("home-screen");
  hs2.classList.remove("hidden"); hs2.style.display="";
};
...
async function loadServersForPicker(){
  ...
  let data = null;
  try{
    // Try preloaded first, then fetch
    if(typeof _preloadedServers !== "undefined" && _preloadedServers){
      data = _preloadedServers;
      // Refresh in background
      sb.from("employees").select("*").eq("active",true).not("role","in","(admin,gerant,Admin,Gerant,Gérant,gérant)").order("name").then(r=>{if(typeof _preloadedServers!=="undefined")_preloadedServers=r.data||[];});
    } else {
      // Fetch all active employees who can serve (not just role=serveur)
      const res=await sb.from("employees").select("*").eq("active",true).not("role","in","(admin,gerant,Admin,Gerant,Gérant,gérant)").order("name");
      data = res.data || [];
      if(typeof _preloadedServers !== "undefined") _preloadedServers = data;
    }
  }catch(e){
    // Fallback: load serveurs only
    try{
      const res2=await sb.from("employees").select("*").eq("active",true).eq("role","serveur").order("name");
      data = res2.data || [];
    }catch(e2){ data=[]; }
  }
  grid.innerHTML="";
  if(!data || !data.length){
    grid.innerHTML=`<div style="text-align:center;padding:40px;color:var(--ink2);font-size:13px;">Aucun serveur actif.<br>Ajoute des serveurs dans Admin → Serveurs.</div>`;
    return;
  }
  (data||[]).forEach(e=>{
...
function enterCaisseOnly(){
  const ap2=document.getElementById("app");
  ap2.classList.add("active"); ap2.style.display="flex";
...
document.getElementById("btn-admin-link").onclick=()=>{
  ...
  const als2=document.getElementById("admin-login-screen");
  als2.classList.add("active"); als2.style.display="flex";
};
document.getElementById("admin-pin-back").onclick=()=>{
  const als3=document.getElementById("admin-login-screen");
  als3.classList.remove("active"); als3.style.display="none";
  const hs3=document.getElementById("home-screen");
  hs3.classList.remove("hidden"); hs3.style.display="";
};
...
async function loadProducts(){
  const {data}=await sb.from("products").select("*,categories(*)").eq("active",true).order("name");
  products=data||[];
  renderProductGrid();
}
...
// ── PRELOAD — instant server screen ──
let _preloadedServers = null;
async function preloadServers(){
  try{
    const {data}=await sb.from("employees").select("*").eq("active",true)
      .in("role",["serveur","admin","gerant"]).order("name");
    _preloadedServers = data || [];
  }catch(e){ _preloadedServers = null; }
}
setTimeout(preloadServers, 2000);
