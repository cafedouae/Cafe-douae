...
async function renderRapportTotals(all, periodLabel) {
  ...
  items.forEach(it => {
    ...
    const shift = isMatin(tk.created_at) ? "matin" : "soir";
    if (isMatin(tk.created_at)) bySection[sec].matin += val;
    else if (isSoir(tk.created_at)) bySection[sec].soir += val;
    const srvName = tk.employees?.name || "—";
    if (!bySrvShift[srvName]) bySrvShift[srvName] = { matin: 0, soir: 0 };
    bySrvShift[srvName][shift] += val;
  ...
  renderSrvShiftTable(bySrvShift, "rp-detail-terrasse");
}

function renderSrvShiftTable(bySrvShift, afterId) {
  const el = ensureDetailBlock("rp-detail-srv-shift", afterId);
  const entries = Object.entries(bySrvShift).sort((a, b) => (b[1].matin + b[1].soir) - (a[1].matin + a[1].soir));
  el.innerHTML = `
    <h3 class="disp" style="margin:0 0 12px;font-size:15px;">🕐 Détail par serveur et par shift</h3>
    ${entries.length ? `
    <table style="width:100%;border-collapse:collapse;font-size:13px;">
      <thead>
        <tr style="border-bottom:1.5px solid var(--line,#eee);text-align:left;">
          <th style="padding:6px 4px;">Serveur</th>
          <th style="padding:6px 4px;text-align:right;">Matin</th>
          <th style="padding:6px 4px;text-align:right;">Soir</th>
          <th style="padding:6px 4px;text-align:right;">Total</th>
        </tr>
      </thead>
      <tbody>
        ${entries.map(([name, v]) => `
          <tr style="border-bottom:1px solid var(--line,#f0f0f0);">
            <td style="padding:6px 4px;"><b>${name}</b></td>
            <td class="mono" style="padding:6px 4px;text-align:right;">${dh(v.matin)}</td>
            <td class="mono" style="padding:6px 4px;text-align:right;">${dh(v.soir)}</td>
            <td class="mono" style="padding:6px 4px;text-align:right;font-weight:700;">${dh(v.matin + v.soir)}</td>
          </tr>`).join("")}
      </tbody>
    </table>` : `<div class="empty">Pas encore de données.</div>`}`;
}
