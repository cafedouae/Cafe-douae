...
async function viewTicket(id){
  ...
  openModal(`
    ...
    <div style="color:var(--ink2);font-size:13px;margin-bottom:12px;">${fmtTime(tk.created_at)} · ${tk.employees?.name||"—"} · ${tk.payment_method||""}</div>
...
async function loadRapportDay(){
  ...
  const to=todayStr(nextDay)+"T04:00:00"; // la journée commerciale s'étend jusqu'à 4h du matin le lendemain
        ...
        return `<div style="background:var(--bg);border-radius:10px;padding:11px 13px;margin-bottom:7px;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            <div style="display:flex;gap:10px;align-items:center;">
              ...
              <span style="font-size:12px;color:var(--ink2);">${fmtTime(t.created_at)} · ${t.employees?.name||"—"}</span>
            </div>
