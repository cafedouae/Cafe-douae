...
const PC_MODE = window.location.search.includes("pc");
...
function getShift(iso){ const h=new Date(iso).getHours(); return (h>=14||h<4)?"🌆 Soir":"🌅 Matin"; }
...
function startPin(emp){
  selectedProfile=emp;
  pinBuffer="";
  document.getElementById("pin-who").textContent=emp.name;
...
function enterApp(){
  ...
  applyRoles();
  // PC mode: show live orders panel
  if(PC_MODE){
...
/* ================================================================
   NAVIGATION
================================================================ */
const VIEW_TITLES={caisse:"Caisse",tickets:"Tickets",stock:"Stock",produits:"Menu & Produits",serveurs:"Serveurs",dashboard:"Statistiques","rapport-cafe":"Rapport ☕ Café","rapport-terrasse":"Rapport 🏖️ Terrasse"};
...
function switchView(v){
  ...
  if(v==="rapport-cafe"){ setRapportDateToday("cafe"); }
  if(v==="rapport-terrasse"){ setRapportDateToday("terrasse"); }
}
...
async function loadTickets(){
  ...
  const {data}=await q;
  const paid=(data||[]).filter(t=>t.status==="payé");

  // Summary totals
  const totalPeriod=paid.reduce((s,t)=>s+Number(t.total),0);
  const matin=paid.filter(t=>{ const h=new Date(t.created_at).getHours(); return h>=8&&h<14; });
  const soir=paid.filter(t=>{ const h=new Date(t.created_at).getHours(); return h>=14||h<4; });
  const el=id=>document.getElementById(id);
  if(el("tks-total"))  el("tks-total").textContent=dh(totalPeriod);
  if(el("tks-count"))  el("tks-count").textContent=paid.length+" ticket"+(paid.length>1?"s":"");
  if(el("tks-matin"))  el("tks-matin").textContent=dh(matin.reduce((s,t)=>s+Number(t.total),0));
  if(el("tks-matin-c")) el("tks-matin-c").textContent=matin.length+" tickets";
  if(el("tks-soir"))   el("tks-soir").textContent=dh(soir.reduce((s,t)=>s+Number(t.total),0));
  if(el("tks-soir-c")) el("tks-soir-c").textContent=soir.length+" tickets";
...
async function viewTicket(id){
  ...
  const canSeeTotal = true; // totals visible to all
  openModal(`
...
function toggleDashMode(){
  ...
  document.getElementById("dash-date-wrap").style.display=mode==="jour"?"":"none";
  document.getElementById("dash-month-wrap").style.display=mode==="mois"?"":"none";
  const cl=document.getElementById("chart-label");
  if(cl) cl.textContent=mode==="mois"?"CA par jour du mois":"CA par heure";
}
function setDashToday(){
  document.getElementById("dash-mode").value="jour";
  document.getElementById("dash-date").value=todayStr();
  toggleDashMode();
  loadDashboard();
}
async function loadDashboard(){
  const mode = document.getElementById("dash-mode")?.value || "jour";
  const now = new Date();

  let periodStart, periodEnd, periodLabel;

  if(mode === "mois"){
    const mv = document.getElementById("dash-month")?.value || `${now.getFullYear()}-${pad(now.getMonth()+1)}`;
    const [yr,mo] = mv.split("-").map(Number);
    periodStart = new Date(yr,mo-1,1);
    periodEnd   = new Date(yr,mo,0,23,59,59);
    periodLabel = `${periodStart.toLocaleString("fr-FR",{month:"long",year:"numeric"})}`;
  } else {
    const dv = document.getElementById("dash-date")?.value || todayStr();
    periodStart = new Date(dv+"T00:00:00");
    periodEnd   = new Date(dv+"T23:59:59");
    periodLabel = periodStart.toLocaleDateString("fr-FR",{weekday:"long",day:"numeric",month:"long",year:"numeric"});
  }
  ...
  const totalRev=all.reduce((s,t)=>s+Number(t.total),0);
  const avgTicket=all.length?totalRev/all.length:0;
  const matin=all.filter(t=>{ const h=new Date(t.created_at).getHours(); return h>=8&&h<14; });
  const soir=all.filter(t=>{ const h=new Date(t.created_at).getHours(); return h>=14||h<4; });

  document.getElementById("kpi-today").textContent=dh(totalRev);
  document.getElementById("kpi-week").textContent=dh(matin.reduce((s,t)=>s+Number(t.total),0));
  document.getElementById("kpi-month").textContent=dh(soir.reduce((s,t)=>s+Number(t.total),0));
  document.getElementById("kpi-tickets").textContent=all.length;
  document.getElementById("kpi-avg").textContent=dh(avgTicket);
  ...
  // Bar chart — par jour si mois, par heure si jour
  let bars=[];
  if(mode==="mois"){
    const daysInMonth=new Date(periodStart.getFullYear(),periodStart.getMonth()+1,0).getDate();
    for(let d=1;d<=daysInMonth;d++){
      const ds=`${periodStart.getFullYear()}-${pad(periodStart.getMonth()+1)}-${pad(d)}`;
    ...
    bars=hours.map(h=>({
      lbl:`${h}h`,
      val:all.filter(t=>new Date(t.created_at).getHours()===h).reduce((s,t)=>s+Number(t.total),0)
    }));
  }
  const maxBar=Math.max(...bars.map(b=>b.val),1);
  document.getElementById("chart-7days").innerHTML=bars.map(b=>`
    <div class="bar-col">
  ...
  items.forEach(it=>{ const s=prodSecMap[it.product_id]||"☕ Café"; bySec[s]=(bySec[s]||0)+Number(it.subtotal); });
  ...
  // By server
  const bySrv={};
  all.forEach(t=>{ const n=t.employees?.name||"—"; if(!bySrv[n]) bySrv[n]={rev:0,cnt:0}; bySrv[n].rev+=Number(t.total); bySrv[n].cnt++; });
  ...
  document.getElementById("stats-servers").innerHTML=srvEntries.length?srvEntries.map(([name,v])=>{
    ...
    return `<div class="prog-item">
      ...
      <div class="prog-bar"><div class="prog-fill" style="width:${pct}%"></div></div>
      <div style="font-size:11px;color:var(--text3);margin-top:2px;">Moy. ${dh(v.cnt?v.rev/v.cnt:0)} / ticket</div>
    </div>`;
  }).join(""):`<div class="empty">Pas encore de données.</div>`;

  // By shift
  const byShift={"🌅 Matin (8h–14h)":{rev:0,cnt:0},"🌆 Soir (14h–04h)":{rev:0,cnt:0}};
  all.forEach(t=>{ const h=new Date(t.created_at).getHours(); const key=h>=8&&h<14?"🌅 Matin (8h–14h)":(h>=14||h<4)?"🌆 Soir (14h–04h)":null; if(key){ byShift[key].rev+=Number(t.total); byShift[key].cnt++; } });
  ...
  // Top products
  const byProd={};
  items.forEach(it=>{ byProd[it.product_name]=(byProd[it.product_name]||{qty:0,rev:0}); byProd[it.product_name].qty+=Number(it.qty); byProd[it.product_name].rev+=Number(it.subtotal); });
  ...
  // By category
  const byCat={};
  items.forEach(it=>{ const c=prodCatMap[it.product_id]||"Autre"; byCat[c]=(byCat[c]||0)+Number(it.subtotal); });
...
/* ================================================================
   RAPPORT CAFÉ & TERRASSE (modules séparés)
================================================================ */
const COMMISSION = { cafe: 0.10, terrasse: 0.05 };
...
function itemSection(item){
  ...
  return prod&&catSection(prod.category_id)==="terrasse"?"terrasse":"cafe";
}
function setRapportDateToday(sec){
  const id = sec==="cafe"?"rpc-date":"rpt-date";
  document.getElementById(id).value = todayStr();
  loadRapportSection(sec);
}
function setRapportDateYesterday(sec){
  const id = sec==="cafe"?"rpc-date":"rpt-date";
  const y=new Date(); y.setDate(y.getDate()-1);
  document.getElementById(id).value = todayStr(y);
  loadRapportSection(sec);
}

async function loadRapportSection(sec){
  const isCafe = sec==="cafe";
  const dateId = isCafe?"rpc-date":"rpt-date";
  const day = document.getElementById(dateId).value;
  if(!day){ toast("Sélectionne un jour",true); return; }

  const color = isCafe?"var(--copper)":"var(--sage)";
  const prefix = isCafe?"rpc":"rpt";
  const commRate = COMMISSION[sec];
  ...
  const to=todayStr(nextDay)+"T04:00:00";

  const {data:tks}=await sb.from("tickets")
    .select("*,employees(name)").eq("status","payé")
    .gte("created_at",from).lte("created_at",to)
    .order("created_at",{ascending:true});

  if(!tks||!tks.length){
    [`${prefix}-matin`,`${prefix}-soir`,`${prefix}-brut`,`${prefix}-commission`,`${prefix}-net`]
      .forEach(id=>{ const el=document.getElementById(id); if(el) el.textContent="0 DH"; });
    document.getElementById(`${prefix}-servers`).innerHTML=`<div class="empty">Aucune vente ce jour.</div>`;
    document.getElementById(`${prefix}-detail`).innerHTML="";
    return;
  }

  const ids=tks.map(t=>t.id);
  const {data:allItems}=await sb.from("ticket_items").select("*").in("ticket_id",ids);
  const data=tks.map(t=>({...t,items:(allItems||[]).filter(i=>i.ticket_id===t.id)}));

  // ── Totaux ──
  let matin=0, soir=0;
  data.forEach(t=>{
    const mat=isMatin(t.created_at), sv=isSoir(t.created_at);
    t.items.filter(i=>itemSection(i)===sec).forEach(i=>{
      const v=Number(i.subtotal);
      if(mat) matin+=v;
      if(sv) soir+=v;
    });
  });
  const brut=matin+soir;
  const commission=brut*commRate;
  const net=brut-commission;

  document.getElementById(`${prefix}-matin`).textContent=dh(matin);
  document.getElementById(`${prefix}-soir`).textContent=dh(soir);
  document.getElementById(`${prefix}-brut`).textContent=dh(brut);
  document.getElementById(`${prefix}-commission`).textContent="−"+dh(commission);
  document.getElementById(`${prefix}-net`).textContent=dh(net);

  // ── Par serveur ──
  const srvMap={};
  data.forEach(t=>{
    const srv=t.employees?.name||"—";
    if(!srvMap[srv]) srvMap[srv]={matin:0,soir:0,brut:0,tickets:0};
    srvMap[srv].tickets++;
    t.items.filter(i=>itemSection(i)===sec).forEach(i=>{
      const v=Number(i.subtotal);
      srvMap[srv].brut+=v;
      if(isMatin(t.created_at)) srvMap[srv].matin+=v;
      if(isSoir(t.created_at)) srvMap[srv].soir+=v;
    });
  });
  const srvWrap=document.getElementById(`${prefix}-servers`);
  srvWrap.innerHTML=Object.entries(srvMap).filter(([,v])=>v.brut>0).sort((a,b)=>b[1].brut-a[1].brut).map(([name,v])=>{
    const comm=v.brut*commRate;
    return `<div style="background:var(--bg);border-radius:10px;padding:12px 14px;margin-bottom:10px;border-left:3px solid ${color};">
      <div style="display:flex;justify-content:space-between;margin-bottom:10px;">
        <div style="font-weight:700;font-size:15px;">👤 ${name}</div>
        <div style="font-size:11px;color:var(--text3);">${v.tickets} ticket${v.tickets>1?"s":""}</div>
      </div>
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px;font-size:12.5px;text-align:center;">
        <div style="background:var(--surface);border-radius:8px;padding:8px 4px;">
          <div style="font-weight:700;" class="mono">${dh(v.matin)}</div>
          <div style="color:var(--text3);font-size:11px;">🌅 Matin</div>
        </div>
        <div style="background:var(--surface);border-radius:8px;padding:8px 4px;">
          <div style="font-weight:700;" class="mono">${dh(v.soir)}</div>
          <div style="color:var(--text3);font-size:11px;">🌆 Soir</div>
        </div>
        <div style="background:rgba(176,60,60,0.08);border-radius:8px;padding:8px 4px;">
          <div style="font-weight:700;color:var(--rose);" class="mono">−${dh(comm)}</div>
          <div style="color:var(--text3);font-size:11px;">Comm. ${commRate*100}%</div>
        </div>
        <div style="background:rgba(74,122,88,0.1);border-radius:8px;padding:8px 4px;">
          <div style="font-weight:700;color:var(--sage);" class="mono">${dh(v.brut-comm)}</div>
          <div style="color:var(--text3);font-size:11px;">💰 Net</div>
        </div>
      </div>
    </div>`;
  }).join("")||`<div class="empty">Aucune vente ${sec==="cafe"?"café":"terrasse"} ce jour.</div>`;

  // ── Détail commandes ──
  const shifts=[
    {label:"🌅 Matin (8h–14h)", filter:t=>isMatin(t.created_at)},
    {label:"🌆 Soir (14h–04h)", filter:t=>isSoir(t.created_at)}
  ];
  const detailWrap=document.getElementById(`${prefix}-detail`);
  detailWrap.innerHTML=`<div class="card" style="border-top:3px solid ${color};">
    <h3 style="margin:0 0 14px;font-size:15px;color:${color};">Détail des commandes</h3>
    ${shifts.map(shift=>{
      const tickets=data.filter(shift.filter).filter(t=>t.items.some(i=>itemSection(i)===sec));
      if(!tickets.length) return `<div style="color:var(--text3);font-size:13px;padding:8px 0;">${shift.label} — Aucune vente</div>`;
      const shiftTotal=tickets.reduce((s,t)=>s+t.items.filter(i=>itemSection(i)===sec).reduce((x,i)=>x+Number(i.subtotal),0),0);
      return `<div style="margin-bottom:16px;">
        <div style="display:flex;justify-content:space-between;font-size:13px;font-weight:600;padding:6px 0;border-bottom:1px solid var(--border);margin-bottom:8px;">
          <span>${shift.label}</span><span class="mono">${dh(shiftTotal)}</span>
        </div>
        ${tickets.map(t=>{
          const secItems=t.items.filter(i=>itemSection(i)===sec);
          ...
          return `<div style="background:var(--bg);border-radius:8px;padding:10px 12px;margin-bottom:6px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
              <div style="display:flex;gap:10px;">
                <span class="mono" style="font-size:11px;font-weight:700;">${t.ticket_number}</span>
                <span style="font-size:12px;color:var(--text2);">${fmtTime(t.created_at)} · ${t.employees?.name||"—"}</span>
              </div>
              <span class="mono" style="font-weight:700;">${dh(secTotal)}</span>
            </div>
            ${secItems.map(i=>`<div style="display:flex;justify-content:space-between;font-size:12.5px;padding:3px 0;border-top:1px dashed var(--border);">
              <span>${i.qty}× ${i.product_name}</span>
              <span class="mono" style="color:var(--text2);">${dh(i.unit_price)} = <b>${dh(i.subtotal)}</b></span>
            </div>`).join("")}
          </div>`;
        }).join("")}
      </div>`;
...
/* ================================================================
   PC MODE — PANNEAU COMMANDES LIVE
================================================================ */
let pcOrders = [];
...
function renderPcPanel(){
  ...
  if(countEl) countEl.textContent=`${pending.length} en attente`;
  if(!pending.length){
    wrap.innerHTML=`<div style="color:#2E7D32;text-align:center;padding:30px;font-size:13px;">✓ Aucune commande</div>`;
    return;
  }
  wrap.innerHTML=pending.map(o=>`
    <div class="pc-order-card${o.isNew?" new-order":""}">
      <div class="pc-order-head">
        <span class="pc-order-num">${o.ticket_number}</span>
        <span class="pc-order-time">${pad(o.time.getHours())}:${pad(o.time.getMinutes())}</span>
      </div>
      <div class="pc-order-srv">👤 ${o.server}</div>
      <div class="pc-order-items">
        ${o.items.map(i=>`<div class="pc-order-item"><span>${i.qty}× ${i.product_name}</span><span>${dh(i.subtotal)}</span></div>`).join("")}
      </div>
      <div class="pc-order-total"><span>Total</span><span style="color:#4CAF50;">${dh(o.total)}</span></div>
      <button class="pc-btn-print" onclick="pcPrint(${o.id})">🖨️ Imprimer + Servi</button>
    </div>`).join("");
}
...
async function pcPrint(orderId){
  ...
  renderPcPanel();
  // Sync with comptoir screen too
  if(typeof comptoirOrders!=="undefined"){
    const co=comptoirOrders.find(x=>x.id===orderId);
    if(co){co.comptoir_status="servi";renderComptoir();}
  }
}
...
/* ================================================================
   COMPTOIR KDS
================================================================ */
let comptoirOrders = []; // {id, ticket_number, server, time, items, new}
  ...
  sb.channel("comptoir-channel")
    .on("postgres_changes", {event:"INSERT", schema:"public", table:"tickets"}, async(payload)=>{
      ...
      const order = {
        ...
        server: emp?.name || "—",
        time: new Date(tk.created_at),
        items: items || [],
        discount: tk.discount || 0,
        total: tk.total || 0,
        payment: tk.payment_method || "—",
        isNew: true,
        comptoir_status: tk.comptoir_status || "pending"
      };
      if(order.comptoir_status === "servi") return;
...
async function loadComptoirOrders(){
  ...
  comptoirOrders = tks
    .filter(t=> t.comptoir_status !== "servi")
    .map(t=>({
      id: t.id,
      ticket_number: t.ticket_number,
      server: t.employees?.name || "—",
      time: new Date(t.created_at),
      items: (allItems||[]).filter(i=>i.ticket_id===t.id),
      discount: t.discount || 0,
      total: t.total || 0,
      payment: t.payment_method || "—",
      isNew: false,
      comptoir_status: t.comptoir_status || "pending"
    }));
  renderComptoir();
}
...
function renderComptoir(){
  ...
  if(!pending.length){
    grid.innerHTML=`<div class="cpt-empty"><div class="big">✓</div><p>Aucune commande en attente</p></div>`;
    return;
  }
...
function printComptoirTicket(orderId){
  ...
  area.innerHTML = `
    ...
    <div style="text-align:center;margin-top:5px;">Merci de votre visite !</div>`;
  window.print();
  // Marquer comme servi après impression
  markServi(orderId);
}
...
================================================================ */
(function init(){
  if(window.location.search.includes("comptoir")){
    initComptoir();
    return;
  }
  buildPinPad();
  ...
  document.getElementById("tk-to").value=todayStr();
  document.getElementById("dash-date").value=todayStr();
  document.getElementById("rpc-date").value=todayStr();
  document.getElementById("rpt-date").value=todayStr();
  document.getElementById("rcp-wifi").textContent=`📶 Wifi : ${WIFI}`;
  loadProfiles();
  const saved=localStorage.getItem("cdouae_user");
  if(saved){ try{ currentUser=JSON.parse(saved); enterApp(); }catch(e){ console.error('enterApp error:',e); } }
})();
window.onerror=function(msg,src,line){ 
  const g=document.getElementById('profiles-grid'); 
  if(g&&!g.innerHTML.trim()) g.innerHTML='<div style="color:red;font-size:13px;padding:10px;">Erreur: '+msg+' ('+src+':'+line+')</div>'; 
  return false;
};
