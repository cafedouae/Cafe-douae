...
async function renderRapportTotals(all, periodLabel) {
  ...
  let items = [];
  if (ids.length) {
    const { data } = await sb.from("ticket_items").select("*").in("ticket_id", ids.slice(0, 900));
    items = data || [];
  }
  ...
  all.forEach(t => {
    ...
    if (!bySrv[n]) bySrv[n] = { rev: 0, cnt: 0 };
    bySrv[n].rev += Number(t.total); bySrv[n].cnt++;
  });
  ...
  document.getElementById("rp-servers").innerHTML = srvEntries.length ? srvEntries.map(([name, v]) => {
    ...
    return `<div class="prog-item">
      <div class="prog-hd"><span><b>${name}</b> <span style="color:var(--text3,#999);font-size:12px;">(${v.cnt} tickets)</span></span><span class="mono" style="color:var(--copper,#C8A040);">${dh(v.rev)}</span></div>
