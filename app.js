...
document.getElementById("server-back").onclick=()=>{
  ...
  if(ss2){ss2.classList.remove("active"); ss2.style.display="none";}
  const hs2=document.getElementById("home-screen");
  if(hs2){hs2.classList.remove("hidden"); hs2.style.display="";}
};
...
async function loadServersForPicker(){
  ...
  try{
    ...
    if(typeof _preloadedServers !== "undefined" && _preloadedServers){
      ...
      sb.from("employees").select("*").eq("active",true).or("role.ilike.%serveur%,role.ilike.%serveuse%").order("name").then(r=>{if(typeof _preloadedServers!=="undefined")_preloadedServers=r.data||[];});
    } else {
      // Fetch all active employees who can serve (not just role=serveur)
      const res=await sb.from("employees").select("*").eq("active",true).or("role.ilike.%serveur%,role.ilike.%serveuse%").order("name");
      data = res.data || [];
