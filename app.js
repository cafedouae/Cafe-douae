...
async function loadTickets(){
  ...
  const soir=paid.filter(t=>{ const h=new Date(t.created_at).getHours(); return h>=14; });
...
async function loadDashboard(){
  ...
  const soir=all.filter(t=>{ const h=new Date(t.created_at).getHours(); return h>=14; });
  ...
  all.forEach(t=>{ const h=new Date(t.created_at).getHours(); const key=h>=8&&h<14?"🌅 Matin (8h–14h)":h>=14?"🌆 Soir (14h–23h)":null; if(key){ byShift[key].rev+=Number(t.total); byShift[key].cnt++; } });
